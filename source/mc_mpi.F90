...
!  modified input so the a planar albedo is supplied by user

!------------------------------------------------------------------
! MPI functionality added by DM Olson June 2015
! compile with,
...
!Chages by D. Olson
!  Changed seed for ran1 so that it will be different everytime code is run
!  and different seed for ever rank.
...
!      
!--------------------------------------------------------------------
! Source code can be compiled by typing (assuming you have Open MPI installed)
!
!  > mpif90 mc_mpi.F90 -o ../bin/mc_mpix
!
!  and run it by typing,
!  > ln -s filepath input_file.in   
! and
!  > mpiexec -np 4 ../bin/mc_mpix
!
!  here filepath is the path to the the specific beam input file. See README for
!  more information.
...
module variables      
  ...
  integer :: nphotpass ! NBR OF PHOTONS THAT PASS THROUGH RINGS WITH NO SCATTERING
  integer :: nphotpassSat, nphotpassSolar ! Decoupled passthrough photons
  ...
  ! SUN
  real*8 :: fluxrcv(nscatter_max) ! recieve buffer for solar flux calculations
  real*8 :: fluxsat(nscatter_max) ! ARRAY FOR FLUX AT EACH SCATTERING -
  !SATSHINE
  real*8 :: fluxsatrcv(nscatter_max) ! recieve buffer for saturn flux calculations
  ...
  integer :: numSolrcv, numSatrcv,passrcv, satpassrcv, solarpassrcv ! buffers for adding photon counts
...
!call MPI_BARRIER(comm,ierror) ! All stop until until read is completed 

if (satswitch == 1) then
      call satshine()
      call MPI_BARRIER(comm,ierror)
      call MPI_REDUCE(fluxsat, fluxsatrcv,20, MPI_REAL8, MPI_SUM,0, comm,ierror)
      fluxsat = fluxsatrcv
endif
   
if (solarswitch == 1) then
      call ray_tracing()   
      call MPI_BARRIER(comm,ierror)
...
call MPI_REDUCE(numfluxSolar, numSolrcv,1, MPI_INT, MPI_SUM,0, comm,ierror)
call MPI_REDUCE(numfluxSat, numSatrcv,1, MPI_INT, MPI_SUM,0, comm,ierror)
numfluxSolar = numSolrcv
numfluxSat = numSatrcv
...
! give full directory for apf file  
  ...
  if (rank==0) write(25,*)'Calculating planar albedo (APF)...'
...
! load k=2 planar flux
  ...
  elseif (  k .eq. k2) then ! it's highly unlikely we will use a k>= 1.15
     ...
     if (rank ==0) write(25,*) 'k value for Apf not found, k= ', k
     stop
  endif
...
subroutine init_grid_speedup()
  ...
  if (rank ==0) write(25,*)'Initialize grid set-up for ring partcles..'
...
subroutine initialize ()
  ...
  if (rank == 0) write(25,*) 'Initializing variables...'
...
! is executed 
  ...
  if (iflag .eqv. .true. ) then
	  ...
	  idum = -1*int(10000*(frac_temp-real(int(frac_temp))))+rank
  else
          idum = -12345+rank
  endif
...
subroutine input ()
  ...
  if (rank == 0) write(25,*) 'input variables'

  if(rank == 0) write(25,*)'Write debug file? 1 = yes, 0 = no'
  read(15,*)idebug
  
  if(rank == 0) write(25,*) 'debug = ', idebug
  

  if (idebug == 1) then
     open (30, file='debug.out',status='replace')
     if (rank==0) write(25,*)'Writing debug file'
     close(30)
  endif

  if(rank==0) write(25,*)'Enter nbr of photons from sun:'
  read(15,*) nphot
  
  if (rank ==0) write(25,*) 'nphot = ', nphot


  if (rank ==0) write(25,*)'Enter sun latitude:'
  read(15,*) sunlat

  if (rank == 0) write(25,*) 'sunlat = ', sunlat


  if (rank == 0) write(25,*)'Enter observer latitude:'
  read(15,*) obslat

  if (rank ==0) write(25,*)'Enter phase angle:'
  read(15,*) alpha

  if (rank==0) write(25,*)'Enter solar hour angle in degress:'
  read(15,*) sha
...
! greater then 1 or less then -1.
  ...
  if ( abs(tempRatio) .gt. 1 ) then
      if (rank == 0) then
        ...
        write(25,*) '**check values for obslat,sunlat and phase angle***'
      endif  
      stop 
 ! **** need to terminate run here**** 
  ...
  if(rank ==0) then
      write(25,*)'sunlat, sunlong, obslat, obslong in Saturn frame:'
      write(25,*)sunlat/raddeg,sunlong/raddeg,obslat/raddeg,obslong/raddeg
  end if
  
  if(rank==0) write(25,*)'Enter wake orientation angle in degrees:'
  read(15,*)phiwake
  ...
  if(rank==0) then
     write(25,*)'sunlat, sunlong, obslat, obslong in ring/wake frame:'
     write(25,*)sunlat/raddeg,sunlongwake/raddeg,obslat/raddeg,obslongwake/raddeg
  end if
...
! VECTOR POINTING TOWARDS OBSERVER IN THE RING-WAKE FRAME
  ...
  ymin = -ymax
  read(10,*)npart
  if (rank == 0) write(25,*)'Number of particles: ',npart
...
! (X, Y DIRRECTIONS LESS CRITICAL SINCE WILL USE GHOST PARTICLES)
  ...
  if (rank==0) then
     write(25,*)'xmin,xmax: ',xmin,xmax
     write(25,*)'ymin,ymax: ',ymin,ymax
     write(25,*)'zmin,zmax: ',zmin,zmax
  end if   
...
! CHOOSE PHASE FUNCTION AND CALCULATE FLUX TO OBSERVER
  write(25,*) idebug,nphot, sunlat, obslat,alpha,sha,phiwake

  write(25,*)'Enter ID nbr for particle phase function :'
  write(25,*)'3 = Power-law spherical phase function for Callisto (n=3.301)'
  write(25,*)'4 = Power-law spherical phase function for Europa'
  write(25,*)'6 = Shadowed-Minnaert surface reflection law - John table'
  write(25,*)'7 = Shadowed-Lambert surface reflection law'
  write(25,*)'8 = Shadowed-Lambert point particle phase function'
  write(25,*)'these number codes are adopted for historical reasons'
...
! OF SATURNSHINE AND/OR FOR DEBUGGING
  ...
  if(rank==0) write(25,*) 'Solar photons on? 0 = off, 1 = on, default = 1'
  read(15,*) solarswitch
  if (rank == 0) write(25,*) 'solarswitch = ', solarswitch

! FOR SATURNSHINE

  if(rank==0) write(25,*) 'Saturnshine? 0 = off, 1 = on, default = 0'
  read(15,*) satswitch
  ...
  if (satswitch == 1) then
     ...
     if(rank==0) write(25,*) 'Enter nbr of lat/long grids on facing Saturn hemisphere:'
     if(rank==0) write(25,*) '(min should be 10 x 10)'
     read(15,*) nsatlat, nsatlong

     if(rank==0) write(25,*) 'Enter distance of ring calculation area from Saturn center in km:'
     read(15,*) ring_dist
...
subroutine ray_tracing ()
  ...
  if(rank==0) write(25,*)'Begin ray tracing of SOLAR photons...'
  if(rank==0) write(25,*)'nphot = ', nphot
  
  lower = rank*(nphot/nproc)+1
  upper = (rank+1)*(nphot/nproc)
  
  do iphot = lower, upper
     if(mod(iphot,100).eq.0) then
        if(rank==0) write(25,*) 'Solar iphot =',iphot
     endif
...
! CHOOSE PHASE FUNCTION FOR SATURN
  ...
  write (25,*) 'What phase func for Saturn?'
  write (25,*) '1 = Barkstrom Law with tabulated A & B'
  write (25,*) '2 = Barkstrom Law with user-chosen A & B'
  write (25,*) '3 = Minnaert'
  read (15,*) satphasefunc
  ...
  enddo
     ...
     write (25,*) 'Which brightness profile table for Saturnshine?'
     write (25,*) '1 = blue belts'
     write (25,*) '2 = blue zones'
     write (25,*) '3 = red belts'
     write (25,*) '4 = red zones'
     read (15,*) isattab
...
!              end if
  ...
  if (satphasefunc == 2) then
     ...
     write (25,*) 'Barkstrom A ~ Saturn albedo'
     read (15,*) a
     write (25,*) 'Barkstrom B ~ amount of backscatter'
     read (15,*) b
...
!              end if
  ...
  if (satphasefunc == 3) then
     ...
     write (25,*) 'Minnaert k?'
     read (15,*) mk
     write (25,*) 'Minnaert albedo for Saturn'
     read (15,*) ma
...
!              end if
  ...
  if(rank==0) write(25,*) 'Total sat photons:', ngridphottot
  if (idebug == 1) then
     open (30, file='debug.out', access='append')
  ...
  if(rank==0) write(25,*)'Begin ray tracing of SATSHINE photons...'
    
  lower = rank*(nphotsat/nproc)+1
  upper = (rank+1)*(nphotsat/nproc)
  
  if(rank==0) write(25,*)'lower, upper : ', lower, upper
