BEAM readme updated January 31, 2018
...
Modules on NAS,
   ...
   If running on Pleiades you will you will have to load the modules 
...
This package contains
	...
	run_mpi4_S0tau05D001Phase60sha45Ntarg300.pbs 
		-this a sample pbs script for BEAM used on NAS it
		executes the fortran and python code mentioned below
		to average over multiple runs.
	...
	runTau10D01S0_Phase20_50targs.sh
	    ...
	    what the pbs script does, but uses far fewer runs. 
	...
	input
	   -sample input files for BEAM 
	      -input files for mcx
			mc-beamS0Phase60sha45tau05D001LitGeom.in
			mc-beamS0Phase60sha45tau05D001LitGeomCal.in
			mc-beamS0Phase60sha45tau05D001LitGeomEur.in
			mc-beamS0Phase60sha45tau05D001LitGeomLam.in
	      -input files for wakex    
			part-no_wakesD001tau05_nbins40.in
			part-wakesDw01taug01tauw02_phi30_sw10hw05_nbins40.in
			part-wakesDw01taug01tauw02_sw10hw05_nbins40.in
			part-wakesDw01taug01tauw05_nbins40.in
			part-wakesDw01taug01tauw05_phi30_nbins40.in	      
          -template files used by python script SumFlux.py    
			  if_sum.out
			  flux_sum.out    
	output
	   -sample output files are in here
	python
	   -python scripts 
		   measureTau.py
		   ...
		   sphericalAlbedo.py
		      ...
		      the paper, "Rough surfaces: Is the dark stuff just shadow?"(2016)
		      by Jeffrey N. Cuzzi, Lindsey B. Chambers, Amanda R. Hendrix
		      
		      Merely used to understand how shadowing and mineart parameters 
		      effect reflectivity.	  
		pythoninfo.txt
...
Compiling code,
    ...
    or the "quad" set up code as,
      ...
      *theoretically, -mcmodel=large can be used but this is not fully implemented
      in gfortran and will not work in a MacOS environment.
...
Running the model,
    ...
    First run the setup code,
      ...
      >../bin/wakex < ../input/part-no_wakesD001tau05_nbins40.in > output_particles
      
      the file output_particles will contain messages normally output to the screen. 
      I use the included file ../input/part-no_wakesD001tau05_nbins40.in as an example. 
      This generates a particle field with the smallest particle with r=5cm and largest 
      with r=500 cm, no wakes, particle sizes are start with the largest and are put in 
      geometrically spaced bins (this example has 40 bins). The physical parameters are 
      tau - 0.5 and D = 0.01. After running you should see a file called:
      nowakes_D001_tau05_nbins40.partdat. This will be used by mcx
      
      Running mcx (the serial version),
      
      type > ../bin/mcx < ../input/mc-beamS0Phase60sha45tau05D001LitGeom.in > output
      
      The file output contains messages printed while the program runs.
      
      In this example the sun latitude an observer latitude are both 10 degrees, phase angle is 
      60 degrees, and the solar hour angle is 45 degrees. The distance to the ring patch is 88000 km with 100000 solar and satshine photons are used. We use the the minneart scatering law as well. 
      There is also a particle roughness parameter but this is set to zero here.
...
running the model on Pleidies,
      ...
      As aluded to above, to speed up execution time we converted the serial version to a MPI version that splits up a large number of 
      photons among multiple processors. Also we found averaging runs among unique particle fields (or targets) with 
      the same physical parameters eliminates bias. So we developed a pbs script that loops through 100 (for high 
      density cases) or 300 (for low density) targets. 
      ...
      When executed the script will put results in folder such as the one you find in /output/mc_mpix_output/
...
running shell script,
      ...
      While in the BASH shell and in the root directory type,
